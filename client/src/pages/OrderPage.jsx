...
const OrderPage = () => {
  ...
  const statusUpdateHandler = async (newStatus) => {
    try {
      ...
      // Simple refresh to show updated status
  ...
  return (
    <div className="container mx-auto mt-8 px-4 animation-fade-in">
      ...
      <p className="text-gray-500 mb-8">Order ID: <span className="font-mono">{order._id}</span></p>
      ...
      <div className="grid lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-1 space-y-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg">
                 <div className="flex items-center gap-3 mb-4"><FaUser className="text-gray-400" /> <h3 className="font-bold text-lg">Customer</h3></div>
             ...
             <div className="bg-white p-6 rounded-lg shadow-lg">
                 <div className="flex items-center gap-3 mb-4"><FaMapMarkerAlt className="text-gray-400" /> <h3 className="font-bold text-lg">Shipping Address</h3></div>
                 <p className="text-gray-700">{order.shippingAddress.address}, {order.shippingAddress.city} {order.shippingAddress.postalCode}, {order.shippingAddress.country}</p>
            </div>
             <div className="bg-white p-6 rounded-lg shadow-lg">
                 <div className="flex items-center gap-3 mb-4"><FaCreditCard className="text-gray-400" /> <h3 className="font-bold text-lg">Payment</h3></div>
                 <p className="text-gray-700 mb-2"><strong>Method:</strong> {order.paymentMethod}</p>
                 <div className={`p-2 rounded-md text-sm text-center font-semibold ${order.isPaid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                    {order.isPaid ? `Paid on ${new Date(order.paidAt).toLocaleDateString()}` : 'Not Paid'}
                 </div>
            </div>

            {/* Admin/Seller Actions */}
            {userInfo && (userInfo.role === 'seller' || userInfo.isAdmin) && order.isPaid && (
              <div className="bg-white p-6 rounded-lg shadow-lg">
