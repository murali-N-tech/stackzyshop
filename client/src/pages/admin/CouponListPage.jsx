  ...
  const createHandler = async (e) => {
    ...
    if (window.confirm('Are you sure you want to create this coupon?')) {
        try {
            ...
            setRefetch(!refetch); // Trigger the list to refetch
            // Reset form
            setCode(''); 
  ...
  const toggleStatusHandler = async (id, currentStatus) => {
    ...
    if (window.confirm(`Are you sure you want to ${action} this coupon?`)) {
      try {
        ...
        setRefetch(!refetch); // Refetch the list to show the updated status
      } catch (err) {
    ...
    <div className="container mx-auto mt-8 px-4">
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1">
          ...
          <form onSubmit={createHandler} className="bg-white p-6 rounded-lg shadow-md space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Coupon Code</label>
              <input type="text" placeholder="e.g., SAVE20" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} required className="mt-1 w-full p-2 border rounded-md" />
            </div>
             <div>
              <label className="block text-sm font-medium text-gray-700">Discount Type</label>
              <select value={discountType} onChange={(e) => setDiscountType(e.target.value)} className="mt-1 w-full p-2 border rounded-md">
             ...
             <div>
              ...
              <input type="number" placeholder={discountType === 'Percentage' ? 'e.g., 10 for 10%' : 'e.g., 50 for $50'} value={discountValue} onChange={(e) => setDiscountValue(e.target.value)} required className="mt-1 w-full p-2 border rounded-md" />
            </div>
             <div>
              <label className="block text-sm font-medium text-gray-700">Expiry Date</label>
              <input type="date" value={expiryDate} onChange={(e) => setExpiryDate(e.target.value)} required className="mt-1 w-full p-2 border rounded-md" />
            </div>
                  ...
                  <tr key={coupon._id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${new Date(coupon.expiryDate) > new Date() && coupon.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {new Date(coupon.expiryDate) > new Date() && coupon.isActive ? 'Active' : 'Expired/Inactive'}
                      </span>
                    </td>
